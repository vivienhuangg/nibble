...
content_id: ac100300274b77f56b550e7edc5d6a1ca173ba5f38a296d51a46538f50d32478
---
...
Deno.test("VersionConcept Actions and Queries", {
  ...
  Deno.test.afterAll(async () => {
    ...
    await db.collection("VersionDraft.drafts").deleteMany({});
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `Effect confirmed: Manual Version ${manualVersionId} created.`,
      );
      ...
      assertEquals(
        retrievedManualVersion.length,
        1,
        "Should find manual version",
      );
      assertEquals(
      ...
      // Note: The version ID from approveDraft is a suggested ID, but createVersion generates a fresh one
      // This is expected behavior as createVersion always generates a new ID
      assertExists(
        (finalApprovedVersion as { version: ID }).version,
        "Version ID from createVersion should exist.",
      );
      console.log(
        `Simulated Sync: createVersion action called with approved draft details.`,
      );

      // Verify the AI-assisted version using the actual ID from createVersion
      const actualVersionId = (finalApprovedVersion as { version: ID }).version;
      ...
      assertEquals(
        retrievedApprovedVersion[0].promptHistory[0].promptText,
        aiGoal,
  ...
  await t.step("draftVersionWithAI action tests", async () => {
    ...
    const goal = "Make it vegan and add a banana.";
    ...
    assert(
      ...
      "Effects: Simulated vegan change for 'vegan' goal (as per internal mock logic).",
    );
    assert(
  ...
  await t.step("approveDraft action tests", async () => {
    ...
    // Test 4: Missing notes in draft details
    const approveResult4 = await concept.approveDraft({
      ...
      draftDetails: {
        ingredients: draftIngredients,
        steps: draftSteps,
        notes: undefined as any, // Missing notes - this will trigger validation error
        goal: draftGoal,
      },
    });
    assertExists(
  ...
  await t.step("Query methods tests", async () => {
    ...
    console.log(
      `Query: _getVersionById(${version1Id}) -> Found version ${
        getResult1[0].versionNum
      }.`,
    );
    ...
    console.log(
      `Query: _getVersionById(${version2Id}) -> Found version ${
        getResult2[0].versionNum
      } with prompt history.`,
    );
    ...
    const errorGetVersion = await concept._getVersionById({
      version: "" as ID,
    });
    assertExists(
