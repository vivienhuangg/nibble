import {
  assertEquals,
  assertExists,
  assertFalse,
  assertNotEquals,
  assertObjectMatch,
  assertRejects,
  assertStrictEquals,
  assertStringIncludes,
} from "jsr:@std/assert";
import { Collection, type Db, type MongoClient } from "npm:mongodb";
import { testDb } from "@utils/database.ts";
import type { ID } from "@utils/types.ts";
import UserConcept from "./UserConcept.ts";

Deno.test("UserConcept: Core User Lifecycle", async (t) => {
  let db: Db;
  let client: MongoClient;
  let userConcept: UserConcept;

  // Setup before the test
  [db, client] = await testDb();
  userConcept = new UserConcept(db);
  console.log("\n--- New Test Scenario ---");

  // Cleanup after the test
  try {
    await t.step("should register a new user successfully", async () => {
      console.log("\nTest: Registering a new user (Alice)");
      const result = await userConcept.registerUser({
        name: "Alice Smith",
        username: "alice",
        password: "password123",
      });

      assertExists(
        (result as { user: ID }).user,
        "User ID should be returned on successful registration.",
      );
      assertFalse(
        (result as { error: string }).error,
        "No error should occur.",
      );

      const userId = (result as { user: ID }).user;
      console.log(`  Registered user with ID: ${userId}`);

      // Verify effects via query
      const userDetails = await userConcept._getUserDetails({ user: userId });
      assertExists(userDetails, "User details should be retrievable.");
      assertFalse(
        (userDetails as { error: string }).error,
        "No error on getting user details.",
      );
      assertEquals(
        (userDetails as Array<{ user: { name: string } }>)[0].user.name,
        "Alice Smith",
        "User name should match.",
      );
      assertEquals(
        (userDetails as Array<{ user: { username: string } }>)[0].user.username,
        "alice",
        "User username should match.",
      );
    });

    await t.step(
      "should prevent registration with an existing username",
      async () => {
        console.log("\nTest: Attempting to register with an existing username");
        // First, register Alice
        await userConcept.registerUser({
          name: "Alice Smith",
          username: "alice2",
          password: "password123",
        });

        // Try to register Bob with Alice's username
        const result = await userConcept.registerUser({
          name: "Bob Johnson",
          username: "alice2",
          password: "securepassword",
        });

        assertExists(
          (result as { error: string }).error,
          "An error should be returned.",
        );
        assertStringIncludes(
          (result as { error: string }).error,
          "username already exists",
          "Error message should indicate existing username.",
        );
        console.log(
          `  Expected error received: ${(result as { error: string }).error}`,
        );
      },
    );

    await t.step("should prevent registration with empty fields", async () => {
      console.log("\nTest: Attempting to register with empty fields");

      const result1 = await userConcept.registerUser({
        name: "",
        username: "testuser",
        password: "pwd",
      });
      assertExists(
        (result1 as { error: string }).error,
        "Error expected for empty name.",
      );
      assertStringIncludes(
        (result1 as { error: string }).error,
        "cannot be empty",
        "Error message should indicate empty fields.",
      );

      const result2 = await userConcept.registerUser({
        name: "Test",
        username: "",
        password: "pwd",
      });
      assertExists(
        (result2 as { error: string }).error,
        "Error expected for empty email.",
      );
      assertStringIncludes(
        (result2 as { error: string }).error,
        "cannot be empty",
        "Error message should indicate empty fields.",
      );

      const result3 = await userConcept.registerUser({
        name: "Test",
        username: "testuser",
        password: "",
      });
      assertExists(
        (result3 as { error: string }).error,
        "Error expected for empty password.",
      );
      assertStringIncludes(
        (result3 as { error: string }).error,
        "cannot be empty",
        "Error message should indicate empty fields.",
      );

      console.log("  Successfully prevented registration with empty fields.");
    });

    await t.step("should log in an existing user successfully", async () => {
      console.log("\nTest: Logging in an existing user (Bob)");
      // Register Bob first
      const registerResult = await userConcept.registerUser({
        name: "Bob Johnson",
        username: "bob",
        password: "securepassword",
      });
      const bobId = (registerResult as { user: ID }).user;
      assertExists(bobId);

      // Now, log in Bob
      const loginResult = await userConcept.login({
        username: "bob",
        password: "securepassword",
      });

      assertExists(
        (loginResult as { user: ID }).user,
        "User ID should be returned on successful login.",
      );
      assertEquals(
        (loginResult as { user: ID }).user,
        bobId,
        "Logged in user ID should match registered ID.",
      );
      assertFalse(
        (loginResult as { error: string }).error,
        "No error should occur.",
      );
      console.log(`  Successfully logged in user with ID: ${bobId}`);
    });

    await t.step("should prevent login with incorrect password", async () => {
      console.log("\nTest: Attempting to log in with incorrect password");
      // Register Charlie
      await userConcept.registerUser({
        name: "Charlie Brown",
        username: "charlie",
        password: "correctpassword",
      });

      // Try to log in with wrong password
      const loginResult = await userConcept.login({
        username: "charlie",
        password: "wrongpassword",
      });

      assertExists(
        (loginResult as { error: string }).error,
        "An error should be returned.",
      );
      assertStringIncludes(
        (loginResult as { error: string }).error,
        "Invalid username or password",
        "Error message should indicate invalid credentials.",
      );
      console.log(
        `  Expected error received: ${
          (loginResult as { error: string }).error
        }`,
      );
    });

    await t.step("should prevent login for non-existent username", async () => {
      console.log("\nTest: Attempting to log in with non-existent username");

      const loginResult = await userConcept.login({
        username: "nonexistent",
        password: "anypassword",
      });

      assertExists(
        (loginResult as { error: string }).error,
        "An error should be returned.",
      );
      assertStringIncludes(
        (loginResult as { error: string }).error,
        "Invalid username or password",
        "Error message should indicate invalid credentials.",
      );
      console.log(
        `  Expected error received: ${
          (loginResult as { error: string }).error
        }`,
      );
    });

    await t.step("should prevent login with empty credentials", async () => {
      console.log("\nTest: Attempting to log in with empty credentials");

      const result1 = await userConcept.login({ username: "", password: "pwd" });
      assertExists(
        (result1 as { error: string }).error,
        "Error expected for empty email.",
      );
      assertStringIncludes(
        (result1 as { error: string }).error,
        "cannot be empty",
        "Error message should indicate empty fields.",
      );

      const result2 = await userConcept.login({
        username: "testuser",
        password: "",
      });
      assertExists(
        (result2 as { error: string }).error,
        "Error expected for empty password.",
      );
      assertStringIncludes(
        (result2 as { error: string }).error,
        "cannot be empty",
        "Error message should indicate empty fields.",
      );

      console.log("  Successfully prevented login with empty credentials.");
    });

    await t.step(
      "should update user name and preferences successfully",
      async () => {
        console.log("\nTest: Updating user name and preferences (Diana)");
        // Register Diana
        const registerResult = await userConcept.registerUser({
          name: "Diana Prince",
          username: "diana",
          password: "wonderwoman",
        });
        const dianaId = (registerResult as { user: ID }).user;
        assertExists(dianaId);

        // Update Diana's name and add a preference
        const updateResult = await userConcept.updateProfile({
          user: dianaId,
          newName: "Diana Wayne",
          newPreferences: { theme: "dark", notifications: true },
        });

        assertFalse(
          (updateResult as { error: string }).error,
          "No error should occur during update.",
        );
        assertObjectMatch(
          updateResult,
          {},
          "Update should return an empty object on success.",
        );

        // Verify effects via query
        const updatedDetails = await userConcept._getUserDetails({
          user: dianaId,
        });
        assertFalse(
          (updatedDetails as { error: string }).error,
          "No error on getting updated user details.",
        );
        assertEquals(
          (updatedDetails as Array<{ user: { name: string } }>)[0].user.name,
          "Diana Wayne",
          "User name should be updated.",
        );
        assertObjectMatch(
          (
            updatedDetails as Array<{
              user: { preferences: Record<string, unknown> };
            }>
          )[0].user.preferences,
          { theme: "dark", notifications: true },
          "Preferences should be updated.",
        );
        console.log(`  User '${dianaId}' updated successfully.`);
      },
    );

    await t.step("should update user username successfully", async () => {
      console.log("\nTest: Updating user username (Eve)");
      const registerResult = await userConcept.registerUser({
        name: "Eve Adams",
        username: "eve_old",
        password: "evepassword",
      });
      const eveId = (registerResult as { user: ID }).user;
      assertExists(eveId);

      const updateResult = await userConcept.updateProfile({
        user: eveId,
        newUsername: "eve_new",
      });

      assertFalse(
        (updateResult as { error: string }).error,
        "No error should occur during update.",
      );

      // Verify effects
      const updatedDetails = await userConcept._getUserDetails({ user: eveId });
      assertEquals(
        (updatedDetails as Array<{ user: { username: string } }>)[0].user.username,
        "eve_new",
        "User username should be updated.",
      );
      console.log(`  User '${eveId}' username updated successfully.`);
    });

    await t.step(
      "should prevent updating username to an already taken one",
      async () => {
        console.log(
          "\nTest: Attempting to update username to an already taken one",
        );
        // Register two users
        const aliceResult = await userConcept.registerUser({
          name: "Alice",
          username: "alice_test",
          password: "pwd",
        });
        const aliceId = (aliceResult as { user: ID }).user;

        await userConcept.registerUser({
          name: "Bob",
          username: "bob_test",
          password: "pwd",
        });

        // Try to change Alice's username to Bob's username
        const updateResult = await userConcept.updateProfile({
          user: aliceId,
          newUsername: "bob_test",
        });

        assertExists(
          (updateResult as { error: string }).error,
          "An error should be returned.",
        );
        assertStringIncludes(
          (updateResult as { error: string }).error,
          "username is already taken",
          "Error message should indicate username is taken.",
        );
        console.log(
          `  Expected error received: ${
            (updateResult as { error: string }).error
          }`,
        );
      },
    );

    await t.step("should prevent updating a non-existent user", async () => {
      console.log("\nTest: Attempting to update a non-existent user");
      const nonExistentId = "nonexistent_user_id" as ID;
      const updateResult = await userConcept.updateProfile({
        user: nonExistentId,
        newName: "Fake User",
      });

      assertExists(
        (updateResult as { error: string }).error,
        "An error should be returned.",
      );
      assertStringIncludes(
        (updateResult as { error: string }).error,
        "User not found",
        "Error message should indicate user not found.",
      );
      console.log(
        `  Expected error received: ${
          (updateResult as { error: string }).error
        }`,
      );
    });

    await t.step("should correctly retrieve user details", async () => {
      console.log("\nTest: Retrieving user details (Frank)");
      const registerResult = await userConcept.registerUser({
        name: "Frank",
        username: "frank",
        password: "frankspassword",
      });
      const frankId = (registerResult as { user: ID }).user;

      const details = await userConcept._getUserDetails({ user: frankId });
      assertFalse(
        (details as { error: string }).error,
        "No error on retrieving details.",
      );
      assertExists(details, "Details should be returned.");
      assertEquals(
        (details as Array<{ user: { name: string } }>).length,
        1,
        "Should return exactly one user.",
      );
      assertEquals(
        (details as Array<{ user: { name: string } }>)[0].user.name,
        "Frank",
      );
      assertEquals(
        (details as Array<{ user: { username: string } }>)[0].user.username,
        "frank",
      );
      assertObjectMatch(
        (details as Array<{ user: { preferences: Record<string, unknown> } }>)[
          0
        ]
          .user.preferences,
        {},
      ); // Initial preferences are empty
      console.log(`  Successfully retrieved details for user '${frankId}'.`);
    });

    await t.step(
      "should return error for non-existent user details query",
      async () => {
        console.log("\nTest: Querying details for a non-existent user");
        const nonExistentId = "fakeid123" as ID;
        const details = await userConcept._getUserDetails({
          user: nonExistentId,
        });
        assertExists(
          (details as { error: string }).error,
          "Error expected for non-existent user.",
        );
        assertStringIncludes(
          (details as { error: string }).error,
          "User not found",
        );
        console.log(
          `  Expected error received: ${(details as { error: string }).error}`,
        );
      },
    );

    await t.step("should correctly retrieve user ID by username", async () => {
      console.log("\nTest: Retrieving user ID by username (Grace)");
      const registerResult = await userConcept.registerUser({
        name: "Grace",
        username: "grace",
        password: "gracepassword",
      });
      const graceId = (registerResult as { user: ID }).user;

      const result = await userConcept._getUserIDByUsername({
        username: "grace",
      });
      assertFalse(
        (result as { error: string }).error,
        "No error on retrieving ID by username.",
      );
      assertExists(result, "Result should be returned.");
      assertEquals(
        (result as Array<{ user: ID }>).length,
        1,
        "Should return exactly one user ID.",
      );
      assertEquals(
        (result as Array<{ user: ID }>)[0].user,
        graceId,
      );
      console.log(`  Successfully retrieved ID for username 'grace'.`);
    });

    await t.step(
      "should return error for non-existent user ID by username query",
      async () => {
        console.log("\nTest: Querying ID by username for a non-existent user");
        const result = await userConcept._getUserIDByUsername({
          username: "unknown",
        });
        assertExists(
          (result as { error: string }).error,
          "Error expected for non-existent username.",
        );
        assertStringIncludes(
          (result as { error: string }).error,
          "User not found",
        );
        console.log(
          `  Expected error received: ${(result as { error: string }).error}`,
        );
      },
    );

    // Trace: Demonstrates the principle "users are the primary actors and owners of content."
    // This principle is demonstrated by a user's ability to register, log in, and manage their own profile,
    // establishing them as a distinct and manageable entity within the system.
    await t.step(
      "trace: User principle fulfillment (Alice's Journey)",
      async () => {
        console.log(
          "\n--- Trace: Alice's Journey to fulfill User concept principle ---",
        );
        console.log(
          "Principle: Users are the primary actors and owners of content.",
        );

        // Alice registers
        console.log("Step 1: Alice registers.");
        const registerResult = await userConcept.registerUser({
          name: "Alice Principle",
          username: "alice_principle",
          password: "securepassword",
        });
        assertFalse(
          (registerResult as { error: string }).error,
          "Registration should succeed.",
        );
        const aliceId = (registerResult as { user: ID }).user;
        assertExists(aliceId);
        console.log(`  Alice registered with ID: ${aliceId}`);

        // Alice logs in
        console.log("Step 2: Alice logs in.");
        const loginResult = await userConcept.login({
          username: "alice_principle",
          password: "securepassword",
        });
        assertFalse(
          (loginResult as { error: string }).error,
          "Login should succeed.",
        );
        assertEquals((loginResult as { user: ID }).user, aliceId);
        console.log(`  Alice logged in successfully.`);

        // Alice updates her profile (demonstrates ownership/control over her content)
        console.log(
          "Step 3: Alice updates her profile with new name and preferences.",
        );
        const updateResult = await userConcept.updateProfile({
          user: aliceId,
          newName: "Alice P. Smith",
          newPreferences: { fav_color: "blue", notifications: false },
        });
        assertFalse(
          (updateResult as { error: string }).error,
          "Profile update should succeed.",
        );
        console.log(`  Alice's profile updated.`);

        // Verify her updated profile
        console.log("Step 4: Verify Alice's updated profile.");
        const updatedDetails = await userConcept._getUserDetails({
          user: aliceId,
        });
        assertFalse(
          (updatedDetails as { error: string }).error,
          "Query for updated details should succeed.",
        );
        assertEquals(
          (updatedDetails as Array<{ user: { name: string } }>)[0].user.name,
          "Alice P. Smith",
          "Name should be updated.",
        );
        assertObjectMatch(
          (
            updatedDetails as Array<{
              user: { preferences: Record<string, unknown> };
            }>
          )[0].user.preferences,
          { fav_color: "blue", notifications: false },
          "Preferences should be updated and merged.",
        );
        console.log(
          "  Alice's updated profile verified. She clearly acts as a primary actor and owner of her data.",
        );
        console.log("--- End Trace: User concept principle fulfilled ---");
      },
    );
  } finally {
    await client.close();
  }
});
